...
C     &                 DIAG3(M-4*NR),
     ...
     &                 VOLDU(NR),VOLDD(NR),VU(NR),VD(NR),KK(NR)
      DOUBLE COMPLEX UX(M,NUDIM),DX(M,NDDIM),ONE,ZERO,
...
C***********************************************************************
      ...
      DOUBLE PRECISION RR(NR)
      DOUBLE COMPLEX UX(NR*NLU,NUDIM),DX(NR*NLD,NDDIM),ONE,ZERO
      PARAMETER (ONE = (1.D0,0.D0),ZERO = (0.D0,0.D0))
...
2        DX(K,I)=ZERO
      ...
      DO 10 J=NU,NU-NOCCU+1,-1
         ...
         POINTER = (NH+L)*NR
         DO 11 K=1,NR
11          UX(POINTER+K,J) = FODA(L,N,RR(K))*RR(K)*ONE
...
10    CONTINUE
      ...
      DO 20 J=ND,ND-NOCCD+1,-1
         ...
         POINTER = (NH+L)*NR
         DO 21 K=1,NR
21          DX(POINTER+K,J) = FODA(L,N,RR(K))*RR(K)*ONE
...
C***********************************************************************
      ...
      INCLUDE 'mode.inc'
      PARAMETER (NUDIM =MAX(NU,1), NDDIM=MAX(ND,1))
      DOUBLE COMPLEX RHOUP(NR,3),
     &              RHODOWN(NR,3),RHO(NR,3),ZETA(NR,3),RR(NR)
      
      DOUBLE COMPLEX UX(NR*NLU,NUDIM),DX(NR*NLU,NDDIM),
     &               ONE,ZERO
      PARAMETER (ONE = (1.D0,0.D0),ZERO = (0.D0,0.D0))
...
C***********************************************************************
      ...
      INCLUDE 'mode.inc'
      PARAMETER (NUDIM =MAX(NU,1), NDDIM=MAX(ND,1))
      DOUBLE PRECISION RR(NR),KK(NR)
      DOUBLE COMPLEX ONE,ZERO,UX(NR*NLU,NUDIM),DX(NR*NLD,NDDIM)
