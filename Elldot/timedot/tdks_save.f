      ...
      INTEGER I,K,NMAX,INFO,IPIV(2),NPCSTEPS,PCCOUNT
      DOUBLE PRECISION T,DN,DN0,V0,V2T,NN,VKS,VV,VXCOLD,VXCNEW
      DOUBLE PRECISION TAU,DTAU,VEXT0,VKS0,VEXT,VXC,OMEGA
      DOUBLE PRECISION A(2,2),W(2),Z(2),NORM   
      DOUBLE COMPLEX ONE,IONE,ZERO,PHI(2),MAT(2,2),R(2),TI,
     &               PHI_OLD(2)
      PARAMETER (ONE=(1.D0,0.D0),IONE=(0.D0,1.D0),ZERO=(0.D0,0.D0))
      PARAMETER (NMAX=20001, NPCSTEPS=2)
      DOUBLE PRECISION V_NUM(NMAX),DN_NUM(NMAX),Y2(NMAX)

      READ(11,*)T,DN0,VEXT0,VKS0,OMEGA

      DO 1 I=NMAX,1,-1
1        READ(50,*)V_NUM(I),DN_NUM(I)
      CALL spline(DN_NUM,V_NUM,NMAX,1.D30,1.D30,Y2)

C**   This is the initial wave function:
      V2T = VKS0/(2.D0*T)
      NORM = ( DSQRT( V2T**2 + 1.D0) - V2T )**2 + 1.D0

      PHI(1) = ONE*( DSQRT(V2T**2 + 1) - V2T)/DSQRT(NORM)
      PHI(2) = ONE/DSQRT(NORM)

C**   Start the time propagation:
      DTAU = 0.01D0
      TAU = 0.D0
      TI = 0.5D0*IONE*DTAU
100   CONTINUE
      ...
      VEXT = VEXT0 + 0.1D0*DSIN(OMEGA*(TAU+DTAU/2.D0))

C**   Predictor-corrector steps

      DN = 2.D0*(PHI(1)*DCONJG(PHI(1)) - PHI(2)*DCONJG(PHI(2)))
      IF (DN.LT.-1.999592D0) DN = -1.99959
      CALL splint(DN_NUM,V_NUM,Y2,NMAX,DN,VV)
      NN = (DN - 2.D0 + DSQRT((2.D0-DN)**2 + 8.D0*DN) )**2
      VKS = -2.D0*T*DSQRT( NN/(16.D0-NN) )
      IF (DN.LT.0.D0) VKS = -VKS
      VXCOLD = VKS-VV
C      VXCOLD = DN/2.D0      
...
C      VXCNEW = DN/2.D0
      ...
      VXC = (VXCOLD + VXCNEW)/2.D0

C**   First calculate the right-hand side

      MAT(1,1) = ONE - TI*(VEXT + VXC)
      MAT(1,2) = TI*T
      MAT(2,1) = TI*T
      MAT(2,2) = ONE 
      ...
      DO 15 I=1,2
         R(I) = ZERO
         DO 15 K=1,2
            R(I) = R(I) + MAT(I,K)*PHI_OLD(K)
15    CONTINUE 

C**   Now do the time propagation step
      ...
      CALL ZGESV( 2, 1, MAT, 2, IPIV, R, 2, INFO )

      PHI(1) = R(1)
      PHI(2) = R(2)

      IF (PCCOUNT.LE.NPCSTEPS) GOTO 101

      WRITE(88,*)REAL(TAU),REAL(DN)

      IF (TAU.LT.300.D0) GOTO 100
