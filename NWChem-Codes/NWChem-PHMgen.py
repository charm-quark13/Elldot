...
# list of excitations to be calculated. If all excitations desired, my_excitations = 'All'
my_excitations = [0,1,2]

def skip(f,lines_to_skip):
...
def GaussianCubeWFLoader(file, skip_first = 0, skip_last = 0, first_orb_in_file = 1):
	"""
...
def GaussianCoeffLoader(file, cutoff = 0.005):
	"""
	...
	for line in f:
		...
		if(if1):
			...
			while True:
				tdline = f.readline()
				if(tdline.find('Occ.')==-1 and tdline.find('Virt.')==-1):
					break
				scrape = tdline.split()
				flag=0
				data=float(scrape[-2])
				...
				if(orb2 > maxorbital):
					maxorbital = orb2
				theexcitation.append([orb1, flag, orb2, data])
...
class GaussianPostProcess(object):
	...
	def CalculateDensityResponse(self, whichexcitation = my_excitations):
		excitation_list = []
	...
	def CalculatePHM(self, whichexcitation = my_excitations, file_type = "pdf", ifnormalize = True):
		excitation_list = []
		if(whichexcitation == 'All'):
			excitation_list = numpy.arange(len(self.coeffs), dtype = int)
		else:
...
gs = GaussianPostProcess("dft-canon.cube", "100-excitations.out", 128, "correct-geo.xyz", first_orb_in_file=1, cutoff = 0.005)
