...
!      x=0.d0
!      do i = 1,n
!         x = x + (hmat(i,1)**4 + hmat(i,2)**4)*(i*delta)
!      end do
!
!      write(*,*) x
!      x=0.d0
!      do i = 1,n
!         x = x + (0.5d0*(hmat(i,1)**4 + hmat(i,2)**4) + 
!     &             3.d0*hmat(i,1)**2*hmat(i,2)**2)*(i*delta)
!      end do
!      write(*,*) x
!
!      counter = 0
...
!         call boys(xi,dboys)
!
!!         write(2000+iter,*) dboys
!
!         if (maxval(dboys).lt.thresh) then
!            exit
!         end if
!
!         call jacobi(dboys,nocc,nocc,jacobiev,rmatdagger,info)
!
!         write(*,*) info
!   
!         do i=1,nocc
!            do j=1,nocc
!               rmat(j,i) = rmatdagger(i,j)
!            end do
!         end do
!   
!         if (iter.eq.1) then
...
!               DO J=1,NOCC
!                  DO K=1,NOCC
!                     xi(i,j) = xi(i,j) + rmatdagger(j,k)*xi(i,k)
!                  end do
!               end do
!            end do
!         end if   
...
!            x=0.d0
!            do k=1,nocc
!               x = x + rmatdagger(i,k)*rmatdagger(j,k)
!            end do
!         write(*,*) i,j,x
!         end do
!      end do
