      ...
      INTEGER N,NOCC,NUN,LWORK,LWORKC,LC,INFO,I,IP,J,JP,K,L,A,AP
      INTEGER NEX
C      PARAMETER (N=200, NOCC=8, NUN=N-NOCC, LC=NOCC*NUN, 
      PARAMETER (N=200, NOCC=7, NUN=20, LC=NOCC*NUN, 
     ...
     &                 WORKC(LWORKC)
      ...
      DOUBLE PRECISION OMEGA_AI(LC),KMAT(LC,LC),KER(N,N)
      DOUBLe PRECISION N1(N),PHM(N,N),TDM(N,N),X,test(lc)
      JOBZ = 'V'
      UPLO = 'U'
...
20    CONTINUE
      ...
      DO 41 JP=1,NUN        
         ...
         KMAT(J+NUN*(I-1),JP+NUN*(IP-1)) = 2.d0* !0.d0* 
...
C*********************************************************************
      ...
      DO 60 K=1,N
         ...
         DO 61 J=1,NUN        
            A = NOCC+J
            N1(K) = N1(K) + HMAT(K,I)*HMAT(K,A)
     &                    *DSQRT(OMEGA_AI(J+NUN*(I-1)))
     &                    *KMAT(J+NUN*(I-1),NEX)
61       CONTINUE
         WRITE(40+10*NEX,*)K,N1(K)
60    CONTINUE
C*********************************************************************
...
C*********************************************************************
      ...
      DO 70 L=1,N
         ...
         DO 71 J=1,NUN        
            A = NOCC+J
            PHM(L,K) = PHM(L,K) + HMAT(L,I)**2*HMAT(K,I)*HMAT(K,A)
     &                    *DSQRT(OMEGA_AI(J+NUN*(I-1)))
     &                    *KMAT(J+NUN*(I-1),NEX)
            TDM(L,K) = TDM(L,K) 
     &       + (HMAT(K,I)*HMAT(L,A)*( DSQRT(OMEGA_AI(J+NUN*(I-1))) 
     &                          + 1.D0/DSQRT(OMEGA_AI(J+NUN*(I-1))))
     &         +HMAT(L,I)*HMAT(K,A)*( DSQRT(OMEGA_AI(J+NUN*(I-1))) 
     &                          - 1.D0/DSQRT(OMEGA_AI(J+NUN*(I-1)))) )
     &                    *KMAT(J+NUN*(I-1),NEX) * 0.5D0
71       CONTINUE
C         WRITE(91,*)L,K,PHM(L,K),TDM(L,K)
70    CONTINUE
...
C75       WRITE(42+10*NEX,900)
      ...
      DO 80 K=1,N
         WRITE(46+10*NEX,901) (PHM(L,K), L=1,N,1)
         WRITE(47+10*NEX,901) (TDM(K,L), L=1,N,1)
80    CONTINUE
...
* Fullsource for module DSYEV from package LAPACK.
* Retrieved from NETLIB on Wed Sep  1 19:11:51 1999.
* ======================================================================
...
* ======================================================================
* NIST Guide to Available Math Software.
* NeededBLAS for module DSYEV from package LAPACK.
* Retrieved from CAMSUN on Wed Sep  1 19:12:50 1999.
* ======================================================================
